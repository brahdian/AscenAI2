    ...
    async def handoff(self, payload: EscalationPayload) -> ConnectorResult:
        ...
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if resp.status_code not in (200, 201):
                logger.error("zendesk_create_ticket_failed", status=resp.status_code, body=resp.text[:300])
                return ConnectorResult(success=False, error=f"Zendesk API {resp.status_code}: {resp.text[:200]}")
