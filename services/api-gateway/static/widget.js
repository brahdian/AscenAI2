  ...
  function sendMessage() {
    ...
    fetchWithRetry(API_URL + '/api/v1/proxy/chat', {
      ...
      .then(function (data) {
        ...
        hideTyping();
        addBotMessage(data.message || 'Sorry, I could not process that.');

        // Handle escalation actions
        if (data.escalation_action === 'chat_handoff') {
          addBotMessage('You are now being connected to a live agent. Please wait…');
        }
      })
      .catch(function (err) {
        hideTyping();
